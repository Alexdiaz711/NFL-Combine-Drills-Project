...
def page_to_rows(page):
    '''
    Takes a page of source code scraped from a web page
    and parses the first table into a list of rows
    ----------
    Parameters
    ----------
    page : str
        A string of a web page source code.
    ----------
    Returns 
    ----------
    A list containing each row in the first table found
    on the web page.
    '''
...
# Connect to mongoDB where the web pages are stored
...
# Creating Pandas DF and initial cleaning of data to create SQL database
combine_df = pd.DataFrame(list(combine2.find()))
draft_df = pd.DataFrame(list(draft2.find()))
...
conn = psycopg2.connect(dbname='', host='localhost', user='postgres', password='')                                            
cur = conn.cursor()                                                                                                                           
